...
**当前状态**：Cycle 1-4、Phase 5、Phase 6、Phase 7-10、Phase 11 已完成并提交；Phase 12 `Google Docs 手感的教师备课桌 UI 重构` 已实现并通过用户验收。当前处于 Cycle 12 Hotfix `数学公式生成、编辑器展示与 Word 导出修复` 验收口。
...
**本轮待验收内容**：
- 含数学公式的生成 / 重写内容不再出现 JSON 控制字符乱码
- 编辑器目标、教学过程、测评题、答案解析、板书等文本字段能展示 LaTeX 公式预览
- 老师仍可编辑原始文本，公式预览随文本更新
- Word 导出会将常见 LaTeX 公式写为可选中、可编辑的 Word 原生公式对象
- 原有语文教案 / 学案生成、改写、确认、引用和导出入口不退化

**验证结果**：公式修复后端全量测试 150 passed，导出测试 12 passed，前端 type-check / test（44 passed）/ build 通过
