...
**当前状态**：Cycle 1-4、Phase 5、Phase 6、Phase 7-10、Phase 11 已完成并提交；当前 Phase 12 `Google Docs 手感的教师备课桌 UI 重构` 已实现并通过用户验收。
...
- 公域页第一屏呈现真实备课文档工作区
- 备课台改为能判断下一步动作的备课队列
- 创建页以课题输入和资料 / 模板匹配为主流程
- 编辑器呈现顶部工具栏、左侧目录、中间纸张画布、右侧检查栏
- 旧教案导入、学校模板、个人资料库同步为文档桌视觉
- 创建、生成、改写、确认、RAG 引用、个人资料引用、质量修复、上课包和 Word 导出链路需回归确认

**验证结果**：Phase 12 前端 type-check / test / build 通过，已在固定 `5173` 端口完成公域首屏浏览器验收
