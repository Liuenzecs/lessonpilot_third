...
**当前状态**：Cycle 1-4、Phase 5、Phase 6 已完成并提交；当前 Phase 7-10 `学校格式与个人备课资产闭环` 已实现，停在用户验收口。
...
**本轮待验收内容**：
- 学校 Word 模板上传、结构化预览、保存和个人隔离
- Word 导出选择个人学校模板，默认导出不退化
- 导出前体检 2.0：目标、过程、评价一致性检查
- 从已确认教案生成学案草稿、PPT 大纲、课堂口播稿
- 个人资料库：`.docx` / `.pptx` 预览、保存、删除和用户隔离
- 原有创建、生成、旧教案导入、RAG 引用、编辑保存和 Word 导出链路需回归确认

**验证结果**：142 pytest + 38 vitest，ruff / type-check / build 全通过
