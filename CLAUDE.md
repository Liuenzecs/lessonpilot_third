...
- 主色使用 Notion Blue，背景以 warm white / pure white 为主
- 边框为 whisper border，阴影克制，优先文档感而不是控制台感
- 公域页、工作台、创建页、编辑器、设置页保持同一套留白与层级节奏
- 暗色模式不作为主流程能力继续扩展
...
**当前状态**：Cycle 1-4、Phase 5、Phase 6、Phase 7-10 已完成并提交；当前 Phase 11 `真实样本验收与个人资料复用闭环` 已实现，停在用户验收口。
...
**本轮待验收内容**：
- 个人资料库推荐：同主题旧 `.docx` / `.pptx` 能在创建页和编辑器被推荐
- 生成时开启“参考我的资料库”，能看到 `asset_status` 命中/未命中状态
- 命中个人资料后，section 级引用显示为“我的资料”，并与公共知识库引用区分
- 其他账号不能推荐、选择或引用当前账号的个人资料
- 导出前体检问题可点击“按建议调整”，生成 pending 修订
- 原有学校模板、旧教案导入、上课包、RAG 引用、编辑保存和 Word 导出链路需回归确认

**验证结果**：146 pytest + 41 vitest，ruff / type-check / build 全通过
