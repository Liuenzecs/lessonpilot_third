...
from app.models import AuthToken, Document, DocumentSnapshot, Feedback, Task, User
...
def get_subscription_summary(session: Session, user: User) -> AccountSubscriptionRead:
    task_count = int(
        session.exec(
            select(func.count())
            .select_from(Task)
            .where(Task.user_id == user.id, Task.created_at >= _month_start())
        ).one()
    )
    return AccountSubscriptionRead(
        ...
        tasks_used_this_month=task_count,
...
def delete_account(session: Session, user: User, payload: AccountDeletePayload) -> None:
    has_delete_text = (payload.confirm_text or "").strip().upper() == "DELETE"
    has_password = bool((payload.password or "").strip())
    if not has_delete_text and (not has_password or not verify_password(payload.password or "", user.password_hash)):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail='Please confirm with "DELETE" or provide the correct password',
        )
